...
public class PlayerInstance
{
    ...
    public void sendThisChunkToPlayer(EntityPlayerMP par1EntityPlayerMP)
    {
        if (this.playersInChunk.contains(par1EntityPlayerMP))
        {
            ...
            if (this.playersInChunk.isEmpty())
            {
                ...
                {
                    PlayerManager.c(this.thePlayerManager).remove(this);
                }
    ...
    public void flagChunkForUpdate(int par1, int par2, int par3)
    {
        if (this.numberOfTilesToUpdate == 0)
        {
            PlayerManager.c(this.thePlayerManager).add(this);
        }
        ...
        if (this.numberOfTilesToUpdate != 0)
        {
            ...
            else
            {
                ...
                if (this.numberOfTilesToUpdate >= ForgeDummyContainer.clumpingThreshold)
                {
                    ...
                    this.sendToAllPlayersWatchingChunk(new Packet51MapChunk(PlayerManager.getWorldServer(this.thePlayerManager).getChunkFromChunkCoords(this.chunkLocation.chunkXPos, this.chunkLocation.chunkZPos), (this.field_73260_f == 0xFFFF), this.field_73260_f, PlayerManager.getWorldServer(this.thePlayerManager).getServer().orebfuscatorUpdateRadius)); // Spigot (Orebfuscator) - CraftBukkit - send everything (including biome) if all sections flagged

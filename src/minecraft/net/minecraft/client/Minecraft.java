...
public abstract class Minecraft implements Runnable, IPlayerUsage
{
    ...
    public void continueWorldLoading()
    {
        ...
        while (!this.theIntegratedServer.serverIsInRunLoop())
        {
            ...
            if (s != null)
            {
                this.loadingScreen.resetProgresAndWorkingMessage(StatCollector.translateToLocal(s));
            }
            else
            {
    ...
    public void loadWorld(WorldClient par1WorldClient, String par2Str)
    {
        ...
        if (par1WorldClient == null)
        {
            ...
            NetClientHandler netclienthandler = this.getSendQueue();
    ...
    public static void fmlReentry(ArgsWrapper wrapper)
    {
        ...
        for (int i = 2; i < par0ArrayOfStr.length; ++i)
        {
            ...
            String s3 = i == par0ArrayOfStr.length - 1 ? null : par0ArrayOfStr[i + 1];
            boolean flag3 = false;
            ...
            if (!s2.equals("-demo") && !s2.equals("--demo"))
            {
                ...
                {
                    String[] astring = HttpUtil.func_82718_a(s, s3);

                    if (astring != null)
                    {
                        s = astring[0];
                        s1 = astring[1];
                        System.out.println("Logged in insecurely as " + s + " - sessionId is " + s1);
                    }
                    else
                    {
                        System.out.println("Could not log in as " + s + " with given password");
                    }
    ...
    private void clickMiddleMouseButton()
    {
        if (this.objectMouseOver != null)
        {
            ...
            if (flag)
            {
                i = this.thePlayer.inventoryContainer.inventorySlots.size() - 9 + this.thePlayer.inventory.currentItem;
                this.playerController.sendSlotPacket(this.thePlayer.inventory.getStackInSlot(this.thePlayer.inventory.currentItem), i);
            }
        }
    }
